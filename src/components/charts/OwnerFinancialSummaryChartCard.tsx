    ...
    >
      <Row gap={8} align="center">
        <View style={[styles.tag, { backgroundColor: '#F0FDF9' }]}>
          <Txt variant="labelSmall" weight="900" color={Colors.primaryDark}>THIS CYCLE</Txt>
        </View>
        <Txt size={11} weight="900" color={Colors.textPrimary} style={{ letterSpacing: 1 }}>P&amp;L SUMMARY</Txt>
      </Row>
      <Txt variant="body" weight="800" color={Colors.textPrimary} style={{ marginTop: 4 }}>Verified collections vs. logged expenses</Txt>

      <Spacer size={14} />
      <Row gap={8}>
        <View style={[styles.kpiBox, { backgroundColor: '#F0FDF9', borderColor: '#CCFBF1' }]}>
          <Row gap={4} align="center"><View style={[styles.legendDot, { backgroundColor: Colors.primary }]} /><Txt variant="labelSmall" color={Colors.primaryDark}>Collected</Txt></Row>
          <Txt variant="cardTitle" weight="900" color={Colors.primaryDark} style={{ marginTop: 2 }}>₹{Math.round(collected).toLocaleString('en-IN')}</Txt>
        </View>
        <View style={[styles.kpiBox, { backgroundColor: '#FFF1F2', borderColor: '#FFE4E6' }]}>
          <Row gap={4} align="center"><View style={[styles.legendDot, { backgroundColor: Colors.accentRose }]} /><Txt variant="labelSmall" color="#B91C1C">Spent</Txt></Row>
          <Txt variant="cardTitle" weight="900" color="#B91C1C" style={{ marginTop: 2 }}>₹{Math.round(spent).toLocaleString('en-IN')}</Txt>
        </View>
        <View style={[styles.kpiBox, { backgroundColor: '#F0F9FF', borderColor: '#E0F2FE' }]}>
          <Row gap={4} align="center"><View style={[styles.legendDot, { backgroundColor: '#0284C7' }]} /><Txt variant="labelSmall" color="#0369A1">Net</Txt></Row>
          <Txt variant="cardTitle" weight="900" color={net >= 0 ? '#0369A1' : '#B91C1C'} style={{ marginTop: 2 }}>₹{Math.round(net).toLocaleString('en-IN')}</Txt>
      ...
      <Spacer size={16} />
      {byCategory.length === 0 ? (
        <Txt variant="caption" color={Colors.textMuted}>No expenses logged this cycle yet.</Txt>
      ) : (
        <Col style={{ gap: 8 }}>
          {byCategory.map((c) => (
            <Row key={c.category} align="center" gap={8}>
              <Txt variant="labelSmall" color={Colors.textSecondary} style={{ width: 78 }}>{CATEGORY_LABEL[c.category] ?? c.category}</Txt>
              <View style={styles.barTrack}>
                <View style={[styles.barFill, { width: `${(c.amount / maxCategory) * 100}%` }]} />
              </View>
              <Txt variant="labelSmall" weight="800" color={Colors.textPrimary} style={{ width: 64, textAlign: 'right' }}>
