...
export function GroceryProductScreen() {
  ...
  const mockedBrand = product.name.split(' ')[0];
  const mockedSubtitle = "Soft, fluffy rotis for a healthier family";
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.sheetContainer}>
          ...
          <Txt maxFontSizeMultiplier={1.2} style={styles.brandText}>{mockedBrand}</Txt>
          ...
          <Txt maxFontSizeMultiplier={1.2} style={styles.subtitleText}>{mockedSubtitle}</Txt>

          {/* Badges Row */}
          <View style={styles.topBadgesRow}>
            <View style={styles.badgesLeft}>
              <View style={styles.ratingBadge}>
                <Txt maxFontSizeMultiplier={1} style={styles.ratingText}>⭐ 4.6 (12.4K reviews)</Txt>
              </View>
              <View style={styles.bestSellerBadge}>
                <Txt maxFontSizeMultiplier={1} style={styles.bestSellerText}>⭐ #1 Best Seller</Txt>
              </View>
            </View>
            <View style={styles.calloutRight}>
              <Ionicons name="leaf" size={24} color="#008040" style={styles.calloutIcon} />
              <Txt maxFontSizeMultiplier={1} style={styles.calloutText}>Made from{'\n'}100% Natural{'\n'}Wheat</Txt>
            </View>
          </View>
          ...
          <View style={styles.benefitStrip}>
            {[
              { label: 'Fresh & Pure\nIngredients', icon: 'leaf-outline' as const },
              { label: 'Trusted\nBrand', icon: 'shield-checkmark-outline' as const },
              { label: 'Rich in\nNutrition', icon: 'nutrition-outline' as const },
              { label: 'Quality\nChecked', icon: 'heart' as const }
            ].map((benefit, index) => (
              <View key={index} style={[styles.benefitItem, index > 0 && styles.benefitDivider]}>
                <Ionicons name={benefit.icon} size={22} color="#008040" />
                <Txt maxFontSizeMultiplier={1} style={styles.benefitItemText}>{benefit.label}</Txt>
              </View>
            ))}
          </View>
          ...
          <View style={styles.detailsBox}>
            ...
            {[
              { label: 'Pack Size', value: selectedOption.unit },
              { label: 'Brand', value: mockedBrand },
              { label: 'Type', value: 'Grocery' },
              { label: 'Shelf Life', value: '6 Months' },
            ].map((row, i) => (
              <View key={i} style={styles.detailRow}>
                <Txt maxFontSizeMultiplier={1.1} style={styles.detailLabel}>{row.label}</Txt>
                <Txt maxFontSizeMultiplier={1.1} style={styles.detailValue}>{row.value}</Txt>
              </View>
            ))}
          </View>
      ...
      <View style={[styles.stickyBar, { paddingBottom: Math.max(insets.bottom, 12) }]}>
        ...
        <AnimatedPress style={styles.stickyAddBtn} onPress={quantity > 0 ? () => router.push('/groceries/cart') : handleAdd}>
          <Ionicons name={quantity > 0 ? 'cart' : 'cart-outline'} size={18} color="#FFF" style={{ marginRight: 8 }} />
...
const styles = StyleSheet.create({
  ...
  headerCircleBtn: {
    width: 38, height: 38, borderRadius: 24,
  ...
  cartBadge: {
    ...
    backgroundColor: '#FF4B4B', borderRadius: 10, minWidth: 16, height: 16,
  ...
  discountBadgeHero: {
    ...
    backgroundColor: '#FF4B4B', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4,
    flexDirection: 'row', alignItems: 'center'
  },
  discountBadgeHeroText: { color: '#FFF', fontSize: 10, fontWeight: '700' },
  paginationDots: {
  ...
  ratingBadge: { backgroundColor: '#FFF9E6', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4 },
  ratingText: { fontSize: 10, fontWeight: '600', color: '#B8860B' },
  bestSellerBadge: { backgroundColor: '#FFF3E0', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4 },
  bestSellerText: { fontSize: 10, fontWeight: '700', color: '#E65100' },
  calloutRight: {
    backgroundColor: '#E8F5E9', borderRadius: 8, padding: 6,
  ...
  savingsPillInline: { backgroundColor: '#E8F5E9', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4, marginLeft: 4 },
  ...
  inlineAddBtn: {
    flex: 1, backgroundColor: '#008040', borderRadius: 24, height: 48,
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center'
  },
  inlineAddText: { color: '#FFF', fontSize: 15, fontWeight: '700' },

  // Benefit Strip
  benefitStrip: {
    flexDirection: 'row', backgroundColor: '#F1F8F5', borderRadius: 8,
  ...
  // Product Details Box
  detailsBox: { borderWidth: 1, borderColor: '#E8E8E8', borderRadius: 12, padding: 16, marginBottom: 24 },
  ...
  stickyAddBtn: {
    backgroundColor: '#008040', borderRadius: 24, height: 44, width: 160,
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center'
  },
  stickyAddText: { color: '#FFF', fontSize: 14, fontWeight: '700' },
  ...
  backBtnError: { backgroundColor: '#008040', borderRadius: 8, paddingVertical: 10, paddingHorizontal: 20, marginTop: 8 },
