  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      <Row justify="space-between" align="center">
        <Txt variant="sectionTitle" weight="900" color={Colors.textPrimary} style={{ letterSpacing: 1 }}>HUB SERVICES & MARKETPLACE</Txt>
      ...
      <Card containerColor={Colors.surface} borderRadius={18} borderWidth={1.5} borderColor={Colors.borderSubtle} padding={[16, 16]}>
        <Row justify="space-between" align="center">
          <Row gap={12} style={{ flex: 1 }}>
            ...
            <Col>
              <Txt variant="cardTitle" weight="900" color={Colors.textPrimary}>EXPRESS PG LAUNDRY</Txt>
              <Txt variant="caption" color={Colors.textSecondary}>Wash & Fold • Wash & Iron • Dry Cleaning</Txt>
            </Col>
          </Row>
        ...
        {myLaundry.length > 0 && (
          <>
            ...
            <Txt variant="caption" weight="700" color={Colors.CyberGreen}>Active Laundry Orders</Txt>
            <Spacer size={8} />
            {myLaundry.slice(0, 2).map((req) => (
              <View key={req.id} style={styles.laundryItem}>
                <Col style={{ flex: 1 }}>
                  <Txt variant="caption" weight="700" color={Colors.textPrimary}>{req.serviceType} • {req.weightOrCount}</Txt>
                  <Txt variant="labelSmall" weight="400" color={Colors.SlateMutedText}>Slot: {req.preferredSlot} • {req.paymentStatus}</Txt>
                </Col>
                <View style={[styles.statusPill, { backgroundColor: req.status === 'Delivered' ? '#ECFDF5' : '#FFFBEB' }]}>
                  <Txt variant="labelSmall" color={req.status === 'Delivered' ? Colors.CyberGreen : Colors.CyberAmber}>{req.status}</Txt>
                </View>
              </View>
            ))}
          </>
      ...
      <Card containerColor={Colors.surfaceElevated} borderRadius={12} borderWidth={1} borderColor={Colors.LuxuryCardBorder} padding={[14, 14]}>
        <Row gap={10} align="center">
          <Txt size={18}>⚡</Txt>
          <Col>
            <Txt variant="caption" weight="700" color={Colors.textPrimary}>PGow Smart Hub Integrated</Txt>
            <Txt variant="labelSmall" weight="400" color={Colors.SlateMutedText} style={{ lineHeight: 14 }}>All services are synchronized directly with your PG landlord's main control panel.</Txt>
          </Col>
        </Row>
      </Card>
...
function HubServiceCard({ title, icon, iconColor, statusText, statusColor, buttonText, onPress }: HubCardProps) {
  return (
    <TouchableOpacity onPress={onPress} style={[styles.hubCard, { flex: 1, minWidth: 140 }]}>
      <Col align="center" style={{ flex: 1, justifyContent: 'space-between' }}>
        <Txt variant="labelSmall" weight="900" color={Colors.IvoryWhiteText} align="center" style={{ lineHeight: 13 }}>{title}</Txt>
        ...
        <View style={[styles.hubStatusPill, { backgroundColor: `${statusColor}26` }]}>
          <Txt variant="labelSmall" color={statusColor}>{statusText}</Txt>
        </View>
