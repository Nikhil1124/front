...
route("POST", "/v1/staff/panic", ({ q, init, body }) => {
  const user = currentUser(init);
  const alert = {
    id: db.genId("panic"), pg_id: q.get("pg_id") ?? db.PG_ID, triggered_by_membership_id: user?.memberships[0]?.membership_id ?? "",
    triggered_by_user_name: user?.name ?? "Staff", latitude: body.latitude, longitude: body.longitude, message: body.message,
    status: "active" as const, created_at: db.nowIso(),
  };
  db.panicAlerts.unshift(alert);
  return ok(alert, 201);
});
route("GET", "/v1/panic/alerts", ({ q }) => {
  const pgId = q.get("pg_id") ?? db.PG_ID;
  const status = q.get("status") ?? "open";
  const rows = db.panicAlerts.filter((a) => a.pg_id === pgId && (status === "open" ? a.status !== "resolved" : a.status === "resolved"));
  return ok(rows);
});
route("POST", "/v1/panic/alerts/:id/acknowledge", ({ params, body }) => {
  const alert = db.panicAlerts.find((a) => a.id === params.id);
  if (!alert) return fail(404, "NOT_FOUND", "Alert not found.");
  (alert as any).status = "acknowledged";
  (alert as any).acknowledged_at = db.nowIso();
  (alert as any).acknowledged_by = body.note ?? "Acknowledged";
  return ok(alert);
});
route("POST", "/v1/panic/alerts/:id/resolve", ({ params, body }) => {
  const alert = db.panicAlerts.find((a) => a.id === params.id);
  if (!alert) return fail(404, "NOT_FOUND", "Alert not found.");
  (alert as any).status = "resolved";
  (alert as any).resolved_at = db.nowIso();
  (alert as any).resolution_note = body.resolution_note;
  return ok(alert);
});

// ── Attendance ───────────────────────────────────────────────────────────

route("GET", "/v1/attendance/shifts", ({ q }) => {
  const pgId = q.get("pg_id") ?? db.PG_ID;
  let rows = db.staffShifts.filter((s) => s.pg_id === pgId);
  const staffId = q.get("staff_membership_id"); if (staffId) rows = rows.filter((s) => s.staff_membership_id === staffId);
  const shiftDate = q.get("shift_date"); if (shiftDate) rows = rows.filter((s) => s.shift_date === shiftDate);
  return ok(rows);
});
route("POST", "/v1/attendance/punch-in", ({ q, body }) => {
  const punch = {
    id: db.genId("punch"), pg_id: q.get("pg_id") ?? db.PG_ID, staff_membership_id: "", shift_id: body.shift_id,
    punch_in_at: db.nowIso(), punch_in_method: body.method ?? "manual", punch_in_latitude: body.latitude, punch_in_longitude: body.longitude,
    status: "in_progress" as const,
  };
  const shift = db.staffShifts.find((s) => s.id === body.shift_id);
  if (shift) punch.staff_membership_id = shift.staff_membership_id;
  db.attendancePunches.push(punch as any);
  return ok(punch, 201);
});
route("POST", "/v1/attendance/punches/:id/punch-out", ({ params, body }) => {
  const punch = db.attendancePunches.find((p) => p.id === params.id);
  if (!punch) return fail(404, "NOT_FOUND", "Punch not found.");
  (punch as any).punch_out_at = db.nowIso();
  (punch as any).punch_out_method = body.method ?? "manual";
  (punch as any).status = "completed";
  return ok(punch);
});
route("GET", "/v1/attendance/punches", ({ q }) => {
  const pgId = q.get("pg_id") ?? db.PG_ID;
  let rows = db.attendancePunches.filter((p) => p.pg_id === pgId);
  const staffId = q.get("staff_membership_id"); if (staffId) rows = rows.filter((p) => p.staff_membership_id === staffId);
  return ok(rows);
});
route("GET", "/v1/attendance/weekly-schedule", ({ q }) => {
  const staffId = q.get("staff_membership_id");
  const weekStart = q.get("week_start_date") ?? new Date().toISOString().slice(0, 10);
  const start = new Date(weekStart);
  const days = Array.from({ length: 7 }).map((_, i) => {
    const d = new Date(start);
    d.setDate(d.getDate() + i);
    const dateStr = d.toISOString().slice(0, 10);
    const shift = db.staffShifts.find((s) => s.staff_membership_id === staffId && s.shift_date === dateStr) ?? (i < 5 ? { ...db.staffShifts.find((s) => s.staff_membership_id === staffId), shift_date: dateStr } : null);
    const punch = db.attendancePunches.find((p) => p.staff_membership_id === staffId) ?? null;
    return { date: dateStr, shift: i < 6 ? shift : null, punch: i === 0 ? punch : null };
  });
  return ok({ week_start_date: weekStart, days });
});
